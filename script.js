...
function cnvrt_t(t, stamp = new Date()) {
    t %= 1;
    let hour = t * 24;
    ...
    stamp.setHours(hour, minute, stamp.getSeconds(), 0);
    stamp = new Date(stamp.getTime() - stamp.getTimezoneOffset() * 60000);
    if (second === 60) {
        stamp.setSeconds(0);
        stamp = new Date(stamp.getTime() + 60000);
    } else {
        stamp.setSeconds(second);
    }
    return stamp;
}
...
function sundial(lat, long, stamp = new Date()) {
    ...
    const millisecond = stamp.getMilliseconds();
    if (month <= 2) {
        year--;
        month += 12;
    }
...
function timeof(angle, lat, long, stamp = new Date(), elev = 0) {
    ...
    if (angle === 'h0') {
        const R = 6378140;
        let d = (Math.abs(elev) * (Math.abs(elev) + 2 * R))**0.5;
        if (elev < 0) {
            d *= -1;
        }
        angle = -0.8333 - asin_d(d / R);
    }
...
function prayer_times(lat, long, stamp = new Date(), elev) {
    ...
    const muntasaf = timeof('h0', lat, long, new Date(stamp.getTime() + 86400000))[2] - 0.5;
    return [fajr18, fajr15, shurooq, dhuhr, asr, maghrib, isha15, isha18, muntasaf]
